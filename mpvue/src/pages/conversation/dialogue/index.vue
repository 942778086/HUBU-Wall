...
// socket.on('connect', function () { console.log('connected successfully') })
...
export default {
    ...
    created() {
        ...
        this.getSend()
        socket.emit('newConnect', {id: this.$store.state.userInfo.id})
        console.log('created')
    },
    methods: {
        sendMsg () {
            ...
            socket.emit('req', {id: socket.id,msg: this.msg})
            this.msg.content = ''
            this.getSend()
        },
        // 第一个执行
        getSend () {
            ...
            this.$fly.post(url + '/getChatByFileds', this.sendData)
                .then(res => {
                    // 这个函数拿到的是拥有send_id的人发送的信息
                    console.log('res:',res)
        ...
        getReceive() {
            ...
            this.$fly.post(url + '/getChatByFileds', this.receiveData)
                .then(res => {
